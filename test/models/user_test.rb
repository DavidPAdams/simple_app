...
class UserTest < ActiveSupport::TestCase
  ...
  test "email validation should accept valid addresses" do
    valid_addresses = %w[user@example.com USER@foo.COM A_US-er@fOO.bar.org first.last@foo.jp alice+bob@ba.cn]
  ...
  test "authenticated? method should return false for a user with nil digest" do
  ...
  test "feed should have correct microposts" do
    michael = users(:michael)
    archer = users(:archer)
    lana = users(:lana)
    # microposts from a followed user
    lana.microposts.each do |post_following|
      assert michael.feed.include?(post_following)      
    end
    # microposts from oneself
    michael.microposts.each do |post_self|
      assert michael.feed.include?(post_self)
    end
    # no microposts from unfollowed user
